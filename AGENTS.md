...
| Tempo analysis / optional Stanza Analyze                     | **[`src/shared/beat/eval/README.md`](src/shared/beat/eval/README.md) first** — the measure-then-change loop that took real-audio Accuracy1 from 20% to 80%; synthetic fixtures cannot settle a tempo question. Then `src/stanza/ANALYZE.md`, `src/shared/beat/TEST_MATRIX.md`, `.agents/rules/beat-analysis-scope.md` | —                              |
...
| **Adding, keeping, or deleting a CI check**                  | [`docs/CI_CHECK_VALUE.md`](docs/CI_CHECK_VALUE.md) **(B)** — placement (blocking / advisory / scheduled / deleted), the solo rule, three-strike, ratchet states; `.agents/rules/ci-check-value.md`                                                                                                                    | —                              |
...
| **Pressure-test a rough plan / half-formed idea**            | [`.agents/skills/labs-grill-plan/SKILL.md`](.agents/skills/labs-grill-plan/SKILL.md) **(B)** — interactive one-question-at-a-time grilling before design/build                                                                                                                                                        | `labs-grill-plan`              |
...
| Retrieving runtime debug artifacts / debugging a running app | [`.agents/rules/dev-debug-artifact-pipe.md`](.agents/rules/dev-debug-artifact-pipe.md) **(B)** — use a `/__debug_*` dev pipe and read the file yourself; never ask the user to forward artifacts                                                                                                                      | —                              |
...
- **Retrieving debug artifacts:** in **local dev**, use a `/__debug_*` dev-server disk pipe and read the file yourself — do not ask the user to forward it ([`.agents/rules/dev-debug-artifact-pipe.md`](.agents/rules/dev-debug-artifact-pipe.md)). In **prod**, use **Copy bundle** in LabsDebugDock; there is no automatic browser→disk/LLM pipe.
