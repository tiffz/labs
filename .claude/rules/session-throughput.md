...
13. **Fix duplicated invariants by searching the CONCEPT, not the expression.** The same off-by-one
    ...
    because of it. Grep every use of the symbol, then move the rule into one function so a sixth
    copy cannot appear. Root cause class: `duplicated-invariant`.
14. **A user-visible bug needs a test at the layer the user sees.** Unit tests were green while
    production stayed broken, because the bug lived in the renderer and the tests covered the
    parser. Reproduce in the browser at the reported URL before claiming a visual bug is fixed.
15. **Codify on second occurrence** of the same root-cause class in-session (test/doc/rule) — see [`docs/CONTINUOUS_PROCESS_IMPROVEMENT.md`](../../docs/CONTINUOUS_PROCESS_IMPROVEMENT.md) § Session throughput.
