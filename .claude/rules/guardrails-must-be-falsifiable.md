...
| `check:ui-copy` (again)        | the em dash ban          | `U+2014` only — `&mdash;` renders one and sailed past      |
| maqam staff-clip e2e           | every maqam × pattern    | one hardcoded URL, the one that happened to pass           |
| maqam ajnas cross-check        | how the two cells relate | each cell against the scale, never the cells to each other |

Each was green while the bug it existed to prevent shipped, and each read as reassurance. The
maqam staff guard is the sharpest case: 29 of 72 combinations clipped their stems, the app's own
default screen among them, and the one URL in the test was not one of the 29.

**A ban is only as wide as its spellings.** If a rule forbids a character, forbid the HTML entity,
the escape, and the decomposed form too — otherwise the rule is one encoding away from unenforced.

## A fifth shape: the proxy the failing case also satisfies

An assertion can be falsifiable, well-scoped, and still wrong because it checks a **stand-in** for
the property, and the broken case satisfies the stand-in.

Maqam's arpeggio claimed "tonic, third, fifth, octave" on a maqam that has no octave. The guard
asserted the top note's `octaveOffset === 1` — but Saba's _seventh_ is also written in the next
octave, so it passed while the claim stayed false. The real property is "the top note is the tonic
an octave up", and asserting that failed immediately, on exactly the one preset.
