...
- `warm-state-optimisation` — an optimisation whose correctness depends on state the developer's machine happens to have and a fresh CI runner never does. Parallelising `skills-ref validate` fired 27 concurrent `npx --yes` processes into one shared npx cache; on a warm cache (any machine that has run it before) they all hit and it looks fine, on a cold cache they race and npm dies with `ENOTEMPTY`. Passed locally, failed on CI's first run. Fix: warm the shared resource once serially before fanning out — and when validating a concurrency or caching change, reproduce the COLD path (`npm_config_cache=<empty dir>`, a clean clone, a fresh container), never the path your machine already primed. Related: `fixture-shares-the-bug`, but the missing dimension is environment state rather than fixture data.
- `local-ci-gate-drift` — a blocking CI job with no presubmit counterpart, so a locally-green run still fails the push ~10 minutes later on a gate the author never saw. `check:react-hooks-ratchet` and `check:agent-guidance` both drifted this way and kept CI red on `main` and `dev-integration` for many consecutive runs; re-adding them cost 7 seconds of presubmit. Now machine-enforced by [`src/shared/ciPresubmitParity.test.ts`](../src/shared/ciPresubmitParity.test.ts). Watch for a stale _reason_ in the exclusion comment — this one said the check "takes >3 min" long after `--cache` had made it ~6s.
- `gate-discovered-by-failing` — learning what a gate wants by tripping it in an expensive hook instead of running the cheap check first. Fix: `npm run verify:quick` (~7s) before committing; treat hooks as a backstop, not a discovery mechanism.
- `guardrail-cannot-fail` — a regression test that passes whether or not the bug is present: constant-vs-constant assertions, circular scoring (judging an output with the machinery that produced it), or a "behavioural" check that silently matches nothing. Four instances shipped real bugs in one session. Fix: [`guardrails-must-be-falsifiable.md`](../.agents/rules/guardrails-must-be-falsifiable.md) — break the code, watch the test fail, then restore.
- `guardrail-proxy-assertion` — a guardrail that is falsifiable and well-scoped, and still wrong, because it asserts a **stand-in** for the property rather than the property, and the broken case satisfies the stand-in. Maqam's arpeggio described "tonic, third, fifth, octave" on Saba, which has no octave; the guard asserted the top degree's `octaveOffset === 1`, but Saba's _seventh_ is also written in the next octave, so it passed while the claim stayed false. Asserting the real property — the top note is the tonic an octave up — failed immediately, on exactly that one preset. Fix: before landing a guard, ask what ELSE makes the assertion true; if the answer includes the bug, it is a proxy. Rule: [`guardrails-must-be-falsifiable.md`](../.agents/rules/guardrails-must-be-falsifiable.md) § A fifth shape.
- `fixture-shares-the-bug` — the test fixture encodes the same wrong assumption as the code, so coverage looks high and the defect is invisible. Every beat fixture was generated at exactly 44100 Hz, the one rate where an 8%-wrong-on-every-song bug cancels; the AudioContext-leak soak seeded a song with no metronome, drums or stems, so it never ran the leaking code. Fix: parametrise over the dimension the bug lives in; seed soak fixtures with the features enabled.
- `silent-wrong-answer` — the system returns a confidently-presented wrong value instead of failing. Essentia read 48 kHz audio as 44.1 kHz and reported every tempo 8% low with **no error and undiminished confidence**; `playNowIfReady` dropped un-decoded hits while the scheduler advanced its cursor. Fix: fail closed at the boundary (assert the precondition and throw), and never let a "skip" advance a cursor. A wrong number presented confidently is worse than no number.
- `per-repeat-allocation` — a loop-repeatable path allocates a scarce resource without releasing it. Stanza minted an `AudioContext` per play **and** per loop wrap in two separate places, and one-shot audio nodes were never disconnected; browsers cap contexts per document, so aux audio went permanently silent before the tab crashed. Fix: allocate once per session, release explicitly, and add a **count-based** test that drives N repeats and asserts the count does not grow.
- `wrapper-worse-than-wrapped` — homegrown post-processing that degrades the library it wraps. 950 lines of tempo heuristics scored 18 points _below_ the single Essentia call underneath them, breaking 9 cases it got right. Fix: measure the wrapper against the bare call before defending it; each heuristic was added to fix one song and never re-validated.
- `two-stores-one-control` — one user-visible control backed by two persisted values that are combined at read time. Stanza's metronome level multiplied a per-song Dexie gain by a global localStorage `masterVolume`, so a slider at 100 played at 50 and neither number matched what you heard. Fix: one persisted value, two views.
- `doc-contradicts-code` — a comment or doc states a behaviour the adjacent code does not implement. A comment promised drums fall back "so sounds still play" three lines above a gate that returned `undefined`; `ARCHITECTURE.md` documented a P0 data-loss bug as the design. Fix: when a comment states a behaviour, that behaviour needs a test; treat doc drift on a data path as a bug, not tidying.
...
- `guardrail-coverage-gap` — a guardrail asserts the right invariant, fails honestly when you break what it covers, and still never covers the thing that breaks, because its file set is hand-maintained. Measured: `check:ui-copy` walked `.tsx` only while the copy lived in `.ts`; `backgroundPlaybackGuardrails` named two drivers while the shared scheduler behind all of them went unchecked; `verify:quick` ran the app typecheck, not `typecheck:full`. All three were green while the bug shipped, and all three read as reassurance. Fix: derive the set from a property of the code, assert it is non-empty, fail loudly when an anchor is renamed, and exempt by name with a reason. Rule: [`guardrails-must-be-falsifiable.md`](../.agents/rules/guardrails-must-be-falsifiable.md) § A fourth shape.
- `silent-durability-failure` — a backup/upload writes locally, fails remotely, and swallows the error, so "saved" and "backed up" become indistinguishable in the UI. Measured: both Encore take-upload paths ended in `catch {}`, so quota exceeded, a revoked token and being offline all rendered as "On this device" — identical to the signed-out state, one device away from loss. Fix: classify the failure ([`describeDriveWriteFailure`](../src/shared/drive/describeDriveWriteFailure.ts)), show the reason where the state is shown, and offer a retry **only when retrying could work**. Guard by deriving the upload sites rather than listing them.
- `squash-resync-deferred` — the squash-promotion resync is done at the next push instead of on the merge, so the trunk conflicts with its own shipped work and it surfaces mid-push on unrelated changes. Four occurrences in one session. The procedure was already written ([`PR_WORKFLOW.md`](PR_WORKFLOW.md) § Long-lived branches must resync); the failure was timing, which is why it now sits in [`ci-background-watch.md`](../.agents/rules/ci-background-watch.md) step 4 next to the merge itself.
...
- `bug-cluster-unescalated` — several bugs landing in one subsystem (or a recurring root-cause class there) were each patched as isolated symptoms, when the cluster was the signal of a shared architectural fragility (e.g. six Encore playback bugs → one dual-clock defect the static tournament never found). Fix: on ≥3 bugs in a subsystem or a recurring class, escalate to a systemic architecture review (`labs-architecture-audit` / `labs-review-architecture`) before patching the next symptom; record the direction as an ADR + invariant guardrails (see `docs/PLAYBACK_ARCHITECTURE_REVIEW_2026-07.md`, ADR 0025).
