...
| Muscle WebGL canvas `toBeVisible` flakes in `test:e2e:smoke`    | 5s default too tight; smoke runs after presubmit under load, WebGL + GLB mount is slow                                                                                   | Use `expectMuscleCanvasReady(page)` (`e2e/helpers/muscleCanvas.ts`, 20s) — never the 5s default for `muscle-training-canvas`; give heavy WebGL describes `test.describe.configure({ timeout: 60_000 })`                                                 |
| Muscle full-body skeleton `Test timeout 90000ms exceeded` on CI | ~400k-tri atlas load + inventory publish runs ~2.5x slower on CI software WebGL (SwiftShader, shared 2-core) than local; 90s cap with retries:0 red-fails the merge gate | CI-aware **caps** in `muscle-full-body-skeleton.spec.ts` (`isCI` → 180s test cap, 120s inventory poll). Caps only bite in the slow tail — the poll resolves on publish — so green runs are unaffected. Do not enable global retries (masks real flakes) |
| Zinebox random unread under load                                | `openReader` only set hash; seed async race                                                                                                                              | Sync `setRoute` in `openReader`; e2e waits for `.zinebox-cover-card` before Random click                                                                                                                                                                |
