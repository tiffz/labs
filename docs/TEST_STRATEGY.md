...
Some feature classes have caused data loss or silent drift before; tests for them are **required,
not optional**. A PR adding one of these features without the named tests is incomplete.

| Feature class                                | Required tests                                                                                                                                                                                                                                                                                                                                          | Reference implementation                                                                     |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **Grid-aligned audio** (metronome, patterns) | Schedule through the platform scheduler (`LookAheadAudioScheduler` / `MetronomeEngine`) — **never `setTimeout`/`setInterval` note clocks** (grep guardrail: `WALL_CLOCK_FORBIDDEN_FILES` in `audioPatternRegistry.test.ts` — add your file when migrating); OfflineAudioContext render test asserting onset positions incl. a mid-playback tempo change | `src/shared/audio/metronome/MetronomeEngine.test.ts`, `scorePlayback.audio.test.ts`          |
| **Audio export** (WAV/MP3 render)            | Deterministic render hash + "output changes when input changes" assertion                                                                                                                                                                                                                                                                               | `src/shared/music/audioCodecs.test.ts`                                                       |
| **Drive sync / merge**                       | Characterization tests: silent-union rows, `needsReview` on both-edited content loss (ADR 0020), schemaVersion mismatch fails loudly, interrupted sidecar upload fails closed                                                                                                                                                                           | `src/zinebox/drive/zineboxDriveConflict.test.ts`, `labsPortfolioDriveHookGuardrails.test.ts` |
| **App-level undo**                           | Tier A: do → undo → redo restores exact state; keyboard bindings                                                                                                                                                                                                                                                                                        | `src/shared/undo/README.md` § tests                                                          |
| **Playback UI wiring** (timing-critical app) | App wiring test with mocked engine: every transport control forwards to the right engine call                                                                                                                                                                                                                                                           | `src/count/App.test.tsx`                                                                     |
