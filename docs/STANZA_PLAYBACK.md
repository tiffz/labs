...
**Non-advancing element (hard EOF):** A media element cannot seek past its own hard `duration`. When a Drive mp4's container end is shorter than the decoded horizon, resuming on the analyzed horizon alone re-clamps to `duration` and re-fires `ended` forever, hanging playback before the timeline end. `resolvePrematureMediaEndResume` takes `previousResumeTargetSec`; if the element ends again at the prior target with no element range past it, we stop at the element's authoritative end. An element that actually advanced (real VBR tail) or grew seekable/buffered still resumes. `useStanzaTransportLoop` clears the stored target only on genuine motion relative to it — the playhead moved past it (`> target + STANZA_PREMATURE_RESUME_STALL_WINDOW_SEC`), landed well before it, the media element changed (song swap), or playback paused. It must **not** clear while the playhead sits at/just below the target — that is the frozen-bug state itself, and clearing there lets the next `ended` resume again. Covered by `useStanzaTransportLoop.prematureEnd.test.ts` (two `ended` events with a tick between).
