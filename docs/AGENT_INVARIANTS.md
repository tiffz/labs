...
| Interactive VexFlow renders gate the first draw on the music font                           | `useVexFlowMusicFontReady` + `useVexFlowMusicFontReady.test.tsx`, `drums-load-interaction.spec.ts` (export bypasses via its own `ensureVexFlowFontsLoaded` preload) |
