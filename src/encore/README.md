...
| `VITE_ENCORE_DRIVE_PUBLIC_PROXY`      | Optional. Set `1` / `true` so **production** guest reads use same-origin `GET /__encore/drive-public/…` (and `…-meta/…`) instead of calling `googleapis.com` directly. Avoids common **CORS failures** after Google redirects `alt=media` to `googleusercontent.com`. Requires an edge proxy in front of your static host (see **Browser API key** and `workers/encore-drive-public-proxy.mjs`). |
| `VITE_ENCORE_DRIVE_PUBLIC_PROXY_BASE` | Optional. When the proxy is **not** on the page origin (e.g. a Worker URL), set to `https://worker.example` (no trailing slash). If unset with `VITE_ENCORE_DRIVE_PUBLIC_PROXY` on, Encore uses `window.location.origin`.                                                                                                                                                                        |
...
**GitHub Pages (CI):** Production builds use the **`VITE_GOOGLE_API_KEY` repository secret** (or the same name on the **`github-pages` environment**). GitHub Actions injects it at build time for `.github/workflows/ci.yml` and `deploy-docs.yml`; it is not read from committed `src/.env.production`. Optional **repository variables** `VITE_ENCORE_DRIVE_PUBLIC_PROXY` and `VITE_ENCORE_DRIVE_PUBLIC_PROXY_BASE` are passed through the same deploy jobs when you enable the guest-drive proxy (see **Browser API key** below). Local dev still uses `src/.env.local` or uncommented vars in `src/.env.development`.
...
**Production guest reads:** A **static** production build that calls `https://www.googleapis.com/drive/v3/files/…?alt=media&key=…` from the browser can still fail with a generic network/CORS error even when the file is public and the key is valid: Google may **redirect** the download to `*.googleusercontent.com`, and that hop often does not expose headers that allow `fetch` to read the body. Fix: deploy a small **same-origin** (or CORS-enabled) **edge proxy** that server-fetches Google and streams the response—same pattern as the Vite dev middleware. A ready-to-adapt script lives at **`workers/encore-drive-public-proxy.mjs`** (Cloudflare Workers–style `fetch` handler). Configure the worker with **`GOOGLE_API_KEY`** (same value as `VITE_GOOGLE_API_KEY`) and **`ENCORE_DRIVE_PROXY_REFERER`** (must match an HTTP referrer allowed on that key, e.g. `https://labs.example.com/encore/`). Route it at **`https://<your-host>/__encore/drive-public/*`** and **`…/drive-public-meta/*`**, then set **`VITE_ENCORE_DRIVE_PUBLIC_PROXY=1`** at build time (and **`VITE_ENCORE_DRIVE_PUBLIC_PROXY_BASE`** if the worker is on another origin). Until that is in place, guest share links may intermittently show “Could not load this snapshot” for otherwise valid snapshots.
