...
function driveVideoFileIdForThumbnail(video: EncorePerformanceVideo): string | null {
  ...
  if (ext) {
    ...
    return video.videoTargetDriveFileId?.trim() || video.videoShortcutDriveFileId?.trim() || null;
  }
  return video.videoTargetDriveFileId?.trim() || video.videoShortcutDriveFileId?.trim() || null;
}
...
export function performanceDriveVideoFileIdForThumbnail(p: EncorePerformance): string | null {
  ...
  return driveVideoFileIdForThumbnail(primary);
}
...
export function performanceVideoThumbnailUrl(p: EncorePerformance): string | null {
  ...
  const id = driveVideoFileIdForThumbnail(primary);
