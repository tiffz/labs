...
- **UI:** `OriginalsSongFilesPanel` on **Song view** (“Song files”) and the **Brainstorm** sidebar (same list, synced), where the dense chip layout fits. The built-in **Brainstorm notes** doc (`brainstormHtml`) always appears as the first chip in the Brainstorm group — source **Encore**, undeletable; click opens the brainstorm editor.
- **Record takes is its own surface.** `OriginalsTakesStage` → `OriginalsTakesWorkspace` renders takes and nothing else — no References, no Brainstorm, no `PracticeResourcesPanel` label rail. A take-card stack sitting on top of two dense chip rows in one label-railed panel was two item metaphors in one scroll; that fork (`takesLayout`) is deleted, not parameterised. Take mutations for **both** surfaces go through `useOriginalsTakes`, so the chip row and the workspace can never drift again.
...
| Playback context churn      | `context/OriginalsChartPlaybackContext.test.tsx`                                           |
