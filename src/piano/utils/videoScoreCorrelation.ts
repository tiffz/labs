...
/**
 * Match BasicPitch note onsets to score notes by pitch class and proximity,
 * then nudge DTW-derived beat times toward the high-confidence anchors.
 *
 * Exported for future "high-accuracy" opt-in mode.  Currently not called
 * by default because BasicPitch inference adds 15-30s to analysis time.
 */
export async function refineBeatsWithPitchAnchors(
  dtwBeats: number[],
  audioBuffer: AudioBuffer,
  score: PianoScore,
  scoreBpm: number,
  ...
  onProgress?.('Refining with pitch detection...');
  await new Promise(r => setTimeout(r, 0));

  let detectPitches: typeof import('../../beat/utils/pitchDetector').detectPitches;
  try {
    const mod = await import('../../beat/utils/pitchDetector');
    detectPitches = mod.detectPitches;
  } catch {
    return dtwBeats;
  }
  ...
  let pitchResult;
  try {
    pitchResult = await detectPitches(audioBuffer);
  } catch {
    return dtwBeats;
  }

  if (!pitchResult.notes.length) return dtwBeats;

  const secPerBeat = 60 / scoreBpm;

  // Build score note onsets: [{beatPos, pitchClass}]
  interface ScoreOnset { beatPos: number; pitchClass: number }
  const scoreOnsets: ScoreOnset[] = [];
  ...
  for (const part of score.parts) {
    ...
    for (const measure of part.measures) {
      ...
      for (const note of measure.notes) {
        ...
        const dur = durationToBeats(note.duration as NoteDuration, note.dotted);
        if (!note.rest && !note.tieStop) {
          for (const p of note.pitches) {
            scoreOnsets.push({ beatPos: beatPos + mb, pitchClass: ((p % 12) + 12) % 12 });
          }
        }
        mb += dur;
      }
      beatPos += Math.max(mb, beatsPerMeasure);
    }
  }

  if (scoreOnsets.length === 0) return dtwBeats;

  // For each score onset, find the DTW-predicted audio time and look for
  // a BasicPitch note with matching pitch class within ±500ms
  interface Anchor { beatPos: number; audioTime: number }
  const anchors: Anchor[] = [];
  const tolerance = 0.5; // seconds

  for (const so of scoreOnsets) {
    const beatIdx = Math.floor(so.beatPos);
    if (beatIdx < 0 || beatIdx >= dtwBeats.length - 1) continue;
    const frac = so.beatPos - beatIdx;
    const predictedAudioTime = dtwBeats[beatIdx] + frac * (dtwBeats[beatIdx + 1] - dtwBeats[beatIdx]);

    let bestDist = tolerance;
    let bestTime = predictedAudioTime;
    for (const pn of pitchResult.notes) {
      if (pn.pitchClass !== so.pitchClass) continue;
      const audioT = pn.startTime;
      const dist = Math.abs(audioT - predictedAudioTime);
      if (dist < bestDist) {
        bestDist = dist;
        bestTime = audioT;
      }
    ...
    if (bestDist < tolerance) {
      anchors.push({ beatPos: so.beatPos, audioTime: bestTime });
    }
  }

  if (anchors.length < 4) return dtwBeats;

  // Sort anchors by beat position
  anchors.sort((a, b) => a.beatPos - b.beatPos);

  // Nudge each DTW beat toward nearby anchors (weighted average)
  const refined = [...dtwBeats];
  for (let b = 0; b < refined.length; b++) {
    // Collect anchors within ±1 beat of this beat
    let weightedSum = 0;
    let totalWeight = 0;

    for (const anc of anchors) {
      const dist = Math.abs(anc.beatPos - b);
      if (dist > 1.5) continue;

      // How much would this anchor shift our beat?
      const anchorBeatFrac = anc.beatPos - b;
      const anchorBeatTime = anc.audioTime - anchorBeatFrac * secPerBeat;

      // Weight: closer anchors contribute more
      const w = 1 / (0.1 + dist);
      weightedSum += anchorBeatTime * w;
      totalWeight += w;
    }

    if (totalWeight > 0) {
      const anchorEstimate = weightedSum / totalWeight;
      // Blend: 70% DTW, 30% anchor (DTW provides global shape, anchors provide local precision)
      refined[b] = 0.7 * refined[b] + 0.3 * anchorEstimate;
    }
  }

  // Ensure monotonicity
  for (let b = 1; b < refined.length; b++) {
    if (refined[b] <= refined[b - 1]) {
      refined[b] = refined[b - 1] + 0.01;
    }
  }
