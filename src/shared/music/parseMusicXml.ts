import type { PianoScore, ScorePart, ScoreMeasure, ScoreNote, ScoreNavigation, NoteDuration, Key, RepeatBarline, VoltaBracket } from './scoreTypes';
import { generateNoteId, durationToBeats } from './scoreTypes';
...
/**
 * Parse partwise MusicXML from an existing {@link Document} (browser DOM or Node linkedom/happy-dom).
 */
export function parseMusicXmlFromDocument(doc: Document): PianoScore & { sections?: ParsedSections[] } {
...
export function parseMusicXml(xmlString: string): PianoScore & { sections?: ParsedSections[] } {
  ...
  return parseMusicXmlFromDocument(doc);
}
