...
4. **Drive backup / local-first sync:** [`../../docs/LOCAL_FIRST_SYNC.md`](../../docs/LOCAL_FIRST_SYNC.md) (§ Portfolio merge prompt policy), [`SHARED_UI_CONVENTIONS.md`](SHARED_UI_CONVENTIONS.md) § Drive, skill `labs-drive-backup`. Shared code: `drive/` (`labsDriveBackupTypes.ts` policy + assessor, auto-sync hook), `google/` (account menu, OAuth).
