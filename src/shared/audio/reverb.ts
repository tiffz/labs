 ...
 * Standalone module that can work with any impulse response
 */
 ...
 * Uses provided impulse response or generates a fallback
 * Enhanced with pre-delay and high-frequency damping for more natural sound
 * @param audioContext - The audio context to use
 * @param impulseResponseUrl - Optional URL to load impulse response from
...
export async function createReverb(
  ...
  if (impulseResponseUrl) {
    try {
      impulseResponse = await loadImpulseResponse(audioContext, impulseResponseUrl);
    } catch {
      console.warn('Could not load impulse response file. Using fallback generated impulse response.');
 ...
 */
export function updateReverbLevel(wetGain: GainNode, dryGain: GainNode, wetLevel: number): void {
  const normalizedWet = Math.max(0, Math.min(1, wetLevel));
