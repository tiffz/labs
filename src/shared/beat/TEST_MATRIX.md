...
| 8–15s drum loop at 120 BPM (synthetic)        | 120                                         | ~60 (half-time) on short Stanza slices        | `shortClipBpm.integration.test.ts` |
| Estimators disagree by octave                 | Normalized weighted consensus + IOI quarter | Locked half or double tempo                   | `tempoEnsemble` + benchmark        |
...
npx vitest run src/shared/beat/analysis/ioiTempoHint.test.ts
RUN_INTEGRATION_TESTS=true npx vitest run src/shared/beat/shortClipBpm.integration.test.ts
npm run test:fast
```
