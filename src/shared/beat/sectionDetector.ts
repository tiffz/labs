...
import { yieldToMainThread } from './decodeMediaForBeat';
import { snapToMeasureStart, generateMeasureLabel } from './measureUtils';
import type { SectionDetectionChordEvent } from './sectionDetectionTypes';

type ChordEvent = SectionDetectionChordEvent;

// Re-export for backwards compatibility
export { extendToMeasureBoundary } from './measureUtils';
export type { SectionDetectionChordEvent as ChordEvent };
...
/** Cap SSM size so long tracks stay responsive (O(n²) novelty pass). */
const MAX_SSM_FRAMES = 2500;
const FEATURE_YIELD_EVERY = 32;
const SSM_YIELD_EVERY_ROWS = 16;
const NOVELTY_YIELD_EVERY = 32;

function resolveSectionFeatureHopSize(channelLength: number, frameSize: number, requestedHop: number): number {
  if (channelLength <= frameSize) return requestedHop;
  const estimatedFrames = Math.floor((channelLength - frameSize) / requestedHop) + 1;
  if (estimatedFrames <= MAX_SSM_FRAMES) return requestedHop;
  const hop = Math.ceil((channelLength - frameSize) / MAX_SSM_FRAMES);
  return Math.max(requestedHop, Math.ceil(hop / 512) * 512);
}
...
async function extractMFCCFeatures(
  ...
  hopSize = resolveSectionFeatureHopSize(channelData.length, frameSize, hopSize);
  ...
  for (let i = 0; i + frameSize <= channelData.length; i += hopSize) {
    if (frameIndex > 0 && frameIndex % FEATURE_YIELD_EVERY === 0) {
 ...
 */
async function buildSelfSimilarityMatrix(features: Float32Array[]): Promise<Float32Array> {
  const n = features.length;
  const ssm = new Float32Array(n * n);

  for (let i = 0; i < n; i++) {
    if (i > 0 && i % SSM_YIELD_EVERY_ROWS === 0) {
      await yieldToMainThread();
    }
    for (let j = 0; j < n; j++) {
 ...
 */
async function detectNovelty(ssm: Float32Array, n: number, kernelSize: number = 32): Promise<Float32Array> {
  const novelty = new Float32Array(n);
  const halfKernel = Math.floor(kernelSize / 2);

  for (let i = halfKernel; i < n - halfKernel; i++) {
    if (i > halfKernel && (i - halfKernel) % NOVELTY_YIELD_EVERY === 0) {
      await yieldToMainThread();
    }
    let sum = 0;
...
export async function detectSections(
  ...
  try {
    ...
    const spectralSSM = await buildSelfSimilarityMatrix(features);
    const n = features.length;
    ...
    const rawNovelty = await detectNovelty(ssm, n, kernelSize);
