...
Local checklist for the right-hand **Practice** rail (metronome, tap tempo, drums, mix). Link here from PRs that touch `StanzaWorkspace`, `StanzaSectionMetronomeRail`, or `StanzaTapTempoDialog` instead of re-explaining these pitfalls in review threads.
