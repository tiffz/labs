...
export function StanzaOrganizeDialogView({
  ...
  const [states, setStates] = useState<StanzaOrganizeGroupSelectionState[]>(() =>
    groups.map(initialStanzaOrganizeGroupState),
  );

  /**
   * Re-seed the per-group selection whenever a fresh scan replaces `groups`. This is React's
   * "adjusting state when a prop changes" pattern — reset during render, not in an effect, so the
   * first paint already shows the new groups instead of one frame of the stale selection.
   */
  const [statesForGroups, setStatesForGroups] = useState(groups);
  if (statesForGroups !== groups) {
    setStatesForGroups(groups);
